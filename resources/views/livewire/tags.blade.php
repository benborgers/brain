...
<div>
    @if (empty($selectedTag))
        ...
        <div class="space-y-3 ml-4">
            @forelse ($tagResults as $tag)
                <div class="cursor-pointer flex items-center space-x-2" wire:click="$set('selectedTag', '{{ $tag }}')">
    ...
    @else
        <div class="flex justify-start">
            <div wire:click="$set('selectedTag', '')" class="flex space-x-1 items-center mb-4 cursor-pointer">
        ...
        <div class="space-y-4 ml-2">
            @foreach ($noteResults as $note)
                <x-note-search-result :note="$note" :highlight="$selectedTag" />
            @endforeach
        </div>
    @endif
